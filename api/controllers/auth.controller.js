...
export const google = async (req, res, next) => {
    try{
        ...
        if (user) {
            ...
            const { password: hashedPassword, ...rest } = user._doc; //remove password
            const expiryDate = new Date(Date.now() + 3600000);
            res.cookie('token', token, {httpOnly: true, expires: expiryDate }).status(200).json(rest);
        }
        else {
            ...
            const newUser = new User(
                    ...
                    profilePicture: req.body.photo}); //if key and value have same name, no need for key
            await newUser.save();
            const token = jwt.sign({ id: newUser._id}, process.env.JWT_SECRET);
            const { password: hashedPassword2, ...rest } = newUser._doc; //remove password
            const expiryDate = new Date(Date.now() + 3600000);
            res.cookie('token', token, {httpOnly: true, expires: expiryDate }).status(200).json(rest);
        }
        }
